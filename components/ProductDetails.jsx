	...
	return (
		<div className="grid grid-cols-2 gap-12">
			...
			<div className="flex max-sm:flex-col-reverse gap-3">
				{allImages.length > 1 && (
					<div className="flex sm:flex-col gap-3 overflow-x-auto sm:overflow-y-auto sm:max-h-112.5">
						{allImages.map((img, i) => (
							<button
								key={i}
				...
				<div className="relative aspect-square sm:w-96 bg-[#F4F5F6] rounded-xl overflow-hidden flex items-center justify-center">
					<div
						className="absolute inset-0 pointer-events-none"
						style={{
							background:
								"radial-gradient(circle at 50% 38%, #FFFFFF, transparent 62%)",
						}}
					/>
					{mainImage ? (
						<Image
						...
						<div className="relative text-slate-400 text-sm">No image</div>
					)}
					{product.category && (
			...
			<div className="flex-1">
				...
				{missingRequired.length > 0 ? (
					<p className="text-3xl font-semibold text-slate-800 mt-4">
						<span className="text-slate-400 font-normal text-lg mr-1.5">
							From
						</span>
				...
				{missingRequired.length > 0 && (
					<div className="flex items-start gap-2 text-xs text-amber-600 bg-amber-50 rounded-lg px-3 py-2.5 mt-1 mb-2 max-w-md">
						...
						<span>
							...
							{missingRequired.map((g) => g.name).join(", ")} from the cart
							page.
						</span>
					</div>
				)}
				...
				{product.description && (
					<p className="mt-8 text-sm text-slate-600 leading-relaxed max-w-lg">
						{product.description}
					</p>
				)}
			</div>
