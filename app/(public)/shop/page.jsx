                ...
                <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-30 max-h-72 overflow-y-auto">
                    <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-sm transition ${
                            !category ? 'text-[var(--primary)] font-medium bg-[var(--primary)]/5' : 'text-slate-600 hover:bg-slate-50'
                        }`}
                    >
                        All categories
                    </button>
                    {categories.map((cat) => (
                        <button
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm transition ${
                                category === cat ? 'text-[var(--primary)] font-medium bg-[var(--primary)]/5' : 'text-slate-600 hover:bg-slate-50'
                            }`}
                        >
                            {cat}
                        </button>
...
function FilterDrawer({ isOpen, onClose, availableTags, initialTags, initialMinPrice, initialMaxPrice, onApply }) {
...
function ShopContent() {
    ...
    const applyDrawerFilters = ({ tags: newTags, minPrice: newMin, maxPrice: newMax }) => {
        const params = new URLSearchParams(searchParams)
        params.delete('tags')
    ...
    const activeFilterCount = tags.length + (minPrice ? 1 : 0) + (maxPrice ? 1 : 0)
